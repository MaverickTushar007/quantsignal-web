...
const RZP_KEY_ID = process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID || "rzp_test_SXLCrBAQeb6qNu";
...
const PLANS = [
  ...
  },
  {
    ...
    features: [
      "Unlimited signals",
      "Unlimited Perseus chat",
      "Telegram alerts",
      "Guardian autonomous monitor",
      "Portfolio tracker + live P&L",
      "All 8 specialist agents",
      "RegimeAgent + ConflictAgent",
      "NewsAgent + CalibrationAgent",
    ],
    locked: ["API access"],
  ...
  },
  {
    ...
    features: [
      ...
      "Priority support",
      "Custom watchlists",
      "Portfolio P&L export",
      "Outcome feedback loop",
      "CalibrationAgent auto-tuning",
    ],
    locked: [],
    cta: "Go Institutional — ₹2,999/mo",
    highlight: false,
  },
];
  ...
  const handleCheckout = async (planId: string) => {
    ...
    try {
      ...
      const options = {
        ...
        name:            "QuantSignal",
        description:     planId === "pro" ? "Pro Plan — ₹999/month" : "Institutional Plan — ₹2,999/month",
        image:           "https://quantsignal.app/logo.png",
        prefill:         { email },
        notes:           { user_id: userId },
        theme:           { color: planId === "pro" ? "#00ff88" : "#aa88ff" },
        handler: async (response: any) => {
          // Payment successful — webhook will update tier, redirect to dashboard
          window.location.href = `/dashboard?upgraded=${planId}&payment_id=${response.razorpay_payment_id}`;
        },
        modal: {
    ...
    <div style={{ minHeight: "100vh", background: "#080a0f", color: "#e2e8f0", fontFamily: "IBM Plex Mono,monospace" }}>
      ...
      <div style={{ maxWidth: 1000, margin: "0 auto", padding: "60px 24px" }}>
        <div style={{ textAlign: "center", marginBottom: 60 }}>
          ...
          <h1 style={{ fontSize: 32, fontWeight: 800, color: "#fff", marginBottom: 12, margin: "0 0 12px" }}>
        ...
        {error && (
          <div style={{ background: "rgba(255,68,102,0.1)", border: "1px solid rgba(255,68,102,0.3)", borderRadius: 10, padding: "12px 16px", marginBottom: 24, fontSize: 11, color: "#ff4466", textAlign: "center" }}>
            {error}
          </div>
        )}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 16 }}>
          {PLANS.map(plan => (
            <div key={plan.id} style={{
              ...
              {plan.highlight && (
                <div style={{ position: "absolute", top: -1, left: "50%", transform: "translateX(-50%)", background: "#00ff88", color: "#000", fontSize: 9, fontWeight: 800, padding: "4px 14px", borderRadius: "0 0 8px 8px", letterSpacing: "0.1em" }}>
                  MOST POPULAR
                </div>
              )}
              ...
              <button
                ...
                style={{
                  ...
                  background: plan.highlight
                    ...
                    ? "linear-gradient(135deg,#aa88ff,#8866cc)"
                  ...
                  fontSize:     11,
                  fontWeight:   700,
                  color:        plan.highlight || plan.id === "institutional" ? "#000" : "rgba(255,255,255,0.6)",
        ...
        <div style={{ textAlign: "center", marginTop: 48, fontSize: 11, color: "rgba(255,255,255,0.3)", lineHeight: 2.2 }}>
          Payments via Razorpay · UPI, Cards, NetBanking accepted · Cancel anytime<br />
