...
const API_BASE_TR = "https://quantsignal-api.onrender.com/api/v1";
...
Give a punchy, honest explanation of why the model made this call, what the market was doing, and what a trader should learn from this.`;
      ...
      const res = await fetch("https://quantsignal-api.onrender.com/api/v1/replay/explain", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const fetchReplay = async (d: string) => {
    ...
    setReplayData(null);
    try {
      const res = await fetch(`https://quantsignal-api.onrender.com/api/v1/signals/${selected.symbol}/replay?replay_date=${d}`);
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16, padding: "8px 0", borderBottom: "1px solid rgba(0,255,136,0.15)", minHeight: 36, background: "rgba(0,255,136,0.03)" }}>
        ...
        {(isPro || true) ? (
          ...
          <button onClick={async () => {
            ...
            const res = await fetch("https://quantsignal-api.onrender.com/api/v1/payments/checkout", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email: user.email, user_id: user.id }) });
